  ...
  return (
    <div className="bg-primary flex flex-col gap-4 rounded-2xl p-5 sm:flex-row sm:items-center sm:gap-6 sm:px-6">
      <div className="min-w-0 flex-1 space-y-1.5">
        <p className="font-mono text-[10px] font-bold tracking-[0.15em] text-white/60">
          LIGA ACTIVA
        </p>
        <h2 className="truncate font-mono text-xl font-bold text-white sm:text-2xl">
          {league.name}
        </h2>
        <p className="truncate text-[13px] text-white/70">{meta}</p>
      </div>
      <Link
        href={`/liga/${league.id}/predicciones`}
        className="text-primary flex shrink-0 items-center justify-center gap-2 rounded-xl bg-white px-5 py-2.5 text-[13px] font-bold transition-colors hover:bg-white/90"
      >
        Ir a predicciones
